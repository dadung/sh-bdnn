...
addpath('liblinear-1.96') % for hinge loss
addpath('liblinear-1.96/matlab/') % for hinge loss



dataset = 'cifar_10_gist320.mat';
...
[Pr2, ~] = evaluate_macro(cateTrainTest, Ret)

hammRadius = 3;
Ret = (hammTrainTest <= hammRadius+0.00001);
[Pr3, ~] = evaluate_macro(cateTrainTest, Ret)

hammRadius = 4;
Ret = (hammTrainTest <= hammRadius+0.00001);
[Pr4, ~] = evaluate_macro(cateTrainTest, Ret)
...
switch dataset
    case 'mnist.mat'
        load('datasets/mnist_gndLabel.mat');
end
precision = KNNPrecision_2(H,tH,k,gnd_inds) * 100
